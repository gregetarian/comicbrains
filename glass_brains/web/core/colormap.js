 ...
 */
export function valueToT(value, maxAbs, mode, gamma = 0.5, divergingMapOnPositive = false) {
    if (mode === 'diverging') {
 ...
 */
export function resolveColormap(style, dataDiverging, colormapsMap) {
    ...
    const name = (!style.colormap || style.colormap === 'auto')
        ...
        : style.colormap;
    // If the chosen LUT is a diverging map but the data is positive-only,
    // remap onto the LUT's hot half so the white centre is never used.
    const cat = colormapsMap && colormapsMap.get(name)?.category;
    const divergingMapOnPositive = mode === 'sequential' && cat === 'diverging';
    return { name, mode, divergingMapOnPositive };
}
 ...
 */
export function colorizeValues(values, cmap, maxAbs, mode, gamma = 0.5, divergingMapOnPositive = false) {
    const n = values.length;
    const out = new Float32Array(n * 3);
    for (let i = 0; i < n; i++) {
        const t = valueToT(values[i], maxAbs, mode, gamma, divergingMapOnPositive);
